/*
  ...
  Copyright (C) supatier 2018
  Modified by github.com/supatier License: MPL 2.0
...
*/
module twitter4d;
import std.digest.hmac, std.digest.sha, std.algorithm, std.datetime,
        std.net.curl, std.base64, std.format, std.string, std.array, std.regex,
        std.stdio, std.json, std.conv;
...
class Twitter4D {
        ...
        this(string consumerKey, string consumerSecret, string accessToken,
        ...
        public auto customUrlRequest(string baseUrl, string type, string endPoint,
        ...
        public auto request(string type, string endPoint, string[string] paramsArgument = null) {
                ...
                string oauthSignature = signature(consumerSecret,
                                accessTokenSecret, method, url, params);
        ...
        public auto stream(string url = "https://userstream.twitter.com/1.1/user.json") {
                ...
                string oauthSignature = signature(consumerSecret,
                                accessTokenSecret, "GET", url, params);
                params["oauth_signature"] = oauthSignature;
        ...
        private string[string] buildParams(string[string] additionalParam = null) {
                ...
                string[string] params = [
                        "oauth_consumer_key" : consumerKey, "oauth_nonce" : randomUUID()
                        .to!string, "oauth_signature_method" : "HMAC-SHA1",
                        "oauth_timestamp" : now, "oauth_token" : accessToken,
                        "oauth_version" : "1.0"
                ];
        ...
        private string signature(string consumerSecret, string accessTokenSecret,
                        string method, string url, string[string] params) {

                auto query = std.algorithm.sort(params.keys).map!(k => k ~ "=" ~ params[k]).join(
                                "&");
                auto key = [consumerSecret, accessTokenSecret].map!(x => encodeComponent(x))
                        .join("&");
                auto base = [method, url, query].map!(x => encodeComponent(x)).join("&");
                string oauthSignature = encodeComponent(Base64.encode(
                                base.representation.hmac!SHA1(key.representation)));
        ...
        string[string] requestAccessToken() {
                string[string] requestToken = toToken(oauthRequest("GET",
                                "request_token").to!string);

                auto authorize_uri = oauthBaseUrl ~ "authorize"
                ...
                string[string] tokens = toToken(oauthRequest("GET",
                                "access_token", ["oauth_verifier" : verifier]).to!string);
...
public static string cleanJSONString(JSONValue input) {
